...
final class SavedRoutesViewModelTests: XCTestCase {
    ...
    func test_publishAsync_withUsername_publishesAndUpdatesRoute() async {
        let context = makeInMemoryContext()
        let social = MockSocialService()
        social.myProfileResult = SocialProfileDTO(id: "u1", username: "gezgin", displayName: nil, isCurator: false, createdAt: .now)
        social.publishResult = "supabase-id-1"
        let badges = MockBadgeServicing()
        let analytics = MockAnalyticsTracking()
        let vm = SavedRoutesViewModel(badges: badges, analytics: analytics, social: social)
        let route = SavedRoute(name: "Sahil Turu", centerLatitude: 41.0, centerLongitude: 29.0, snapshots: [])
        context.insert(route)

        await vm.publishAsync(route, context: context)

        XCTAssertEqual(social.publishCallCount, 1)
        XCTAssertEqual(route.supabaseId, "supabase-id-1")
        XCTAssertTrue(route.isPublic)
        XCTAssertEqual(vm.publishedRouteName, "Sahil Turu")
        XCTAssertEqual(badges.sharedRouteCount, 1)
        XCTAssertEqual(analytics.trackedEvents, [.routePublished])
        XCTAssertFalse(vm.showUsernameSetup)
        XCTAssertNil(vm.publishErrorMessage)
    }

    func test_publishAsync_withoutUsername_showsSetupAndDoesNotPublishYet() async {
        let context = makeInMemoryContext()
        let social = MockSocialService()
        social.myProfileResult = nil
        let vm = SavedRoutesViewModel(badges: MockBadgeServicing(), analytics: MockAnalyticsTracking(), social: social)
        let route = SavedRoute(name: "Sahil Turu", centerLatitude: 41.0, centerLongitude: 29.0, snapshots: [])
        context.insert(route)
        ...
        XCTAssertEqual(social.publishCallCount, 0)
        XCTAssertFalse(route.isPublic)
    }

    func test_performPublish_cityUnresolvedError_setsErrorAndLeavesRoutePrivate() async {
        let context = makeInMemoryContext()
        let social = MockSocialService()
        social.publishError = SocialServiceError.cityUnresolved
        let vm = SavedRoutesViewModel(badges: MockBadgeServicing(), analytics: MockAnalyticsTracking(), social: social)
        let route = SavedRoute(name: "Sahil Turu", centerLatitude: 41.0, centerLongitude: 29.0, snapshots: [])
        context.insert(route)

        await vm.performPublish(route, context: context)

        XCTAssertNotNil(vm.publishErrorMessage)
        XCTAssertFalse(route.isPublic)
        XCTAssertNil(route.supabaseId)
    }

    func test_usernameSetupSucceeded_resumesPendingPublish() async {
        let context = makeInMemoryContext()
        let social = MockSocialService()
        social.myProfileResult = nil
        let vm = SavedRoutesViewModel(badges: MockBadgeServicing(), analytics: MockAnalyticsTracking(), social: social)
        ...
        social.myProfileResult = SocialProfileDTO(id: "u1", username: "gezgin", displayName: nil, isCurator: false, createdAt: .now)
        vm.usernameSetupSucceeded()
        // pendingAction kendi Task'ını spawn eder — kısa bir yield ile tamamlanmasını bekle.
