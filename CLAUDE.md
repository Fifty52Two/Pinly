...
> İlgili belgeler: `ROADMAP.md` (ürün/büyüme planı + test stratejisi), `IMPROVEMENT_PLAN.md` (kod incelemesi bulguları, düzeltilen buglar B1-B15, refactor geçmişi).
...
- Xcode projesi: `Pinly.xcodeproj` — target'lar: `Pinly` (app), `PinlyLiveActivity` (widget extension)
...
| Dosya | İçerik |
|---|---|
| `ServiceEnvironment.swift` | `\.entitlements`, `\.badges`, `\.ads` EnvironmentKey'leri. Default value'lar gerçek singleton'lar → preview'lar kurulum istemez, testte mock inject edilir. |
...
### `Pinly/manager/` — ObservableObject state yöneticileri
| Dosya | Rol |
|---|---|
...
| `locationManager.swift` | `LocationManager` (dosya adı küçük harf!). İzin `requestPermission()` ile ONBOARDING SONRASI istenir (init'te değil). Normalde tek seferlik konum (100 m doğruluk, pil dostu) + reverse geocode → `currentDistrict`. `startNavigationTracking()`: 10 m doğruluk/filtre + 2 saat güvenlik timer'ı. |
...
| `HealthKitManager.swift` | Rota süresince adım+mesafe sorgusu (`fetchRouteStats(from:to:)`). Rota tamamlanınca RouteHistory'e yazılır. |
...
| `PinlyActivityAttributes.swift` | Live Activity ContentState (talimat, kalan mesafe, durak x/y, tamamlanma %). **Her iki target'a da ekli olmalı.** |

### `Pinly/design/Theme.swift` — Tasarım sistemi
"Slate" palet (2026-07 revizyonu, meditasyon-app referansı): slate-indigo aksan (`PinlyTheme.primary`, dark'ta açılır), kısık gül `accent`, `gold`/`slate` destek, açık gri-mavi `ground` (dark'ta koyu slate-navy #131C26), `surface` kart (light beyaz / dark #1D2936). `navy` (#1B2733) tab bar + paylaşım kartı gibi her modda koyu yüzeyler için sabit. heroGradient = aynı ailede iki komşu slate tonu. `PinlyPrimaryButtonStyle`, `PinlySecondaryButtonStyle`, `.pinlyCard()` (gölge yerine ince kontur), `StatChip`. **Yeni bileşenler renkleri BURADAN alır, hardcode etme.** Yeşil YOK — kullanıcı kararı.

### `Pinly/view/` — Ekranlar

**Kök & sekmeler:**
- `ContentView.swift` — onboarding/izin/ana yönlendirme; deep link handler (`addplace` → ImportConfirmView, `route` → RouteImportView, `navigation` → HomeView'e bırakılır); import'larda freemium gate. `HomeView`: 4 sekmeli TabView + rozet banner overlay + `pinly://navigation` ve `pinly://quickadd` handler'ları.
- `view/home/MainTab.swift` — Ana sekme: selamlama, StatChip şeridi, hero "Rota Planla" (→MapView), 4 hızlı aksiyon (Mekanlarım/Mekan Ekle/QR Tara/Rota Tasarla), Son Eklenenler.
...
- `view/home/MoreTab.swift` — Daha Fazla: İstatistiklerim/Geçmiş/Haftalık Rapor/Rozetler/Dil seçici.

**Mekan CRUD:**
- `view/home/PlacesListView.swift` + `PlacesListViewModel.swift` — liste; arama/kategori chip/sıralama mantığı ViewModel'de (saf fonksiyonlar, test edilebilir; sıralama tercihi `pinly.sortOption`). Toolbar: sırala/Swarm import (fileImporter)/QR/ekle(gate). Swipe: sil+paylaş / ziyaret toggle.
- `AddPlaceView.swift` / `EditPlaceView.swift` — form; 3 konum yöntemi: adres yaz (geocode), mevcut konum, **Haritada Pinle** → `MapPinPickerView.swift` (sabit merkez pin, harita altında kayar, kamera durunca reverse geocode — Uber deseni).
- `PlaceDetailView.swift` — salt okunur detay + harita önizleme + Düzenle/Paylaş.
- `QuickAddSheet.swift` — widget deep link'inden hızlı ekleme (konum otomatik).
- `SharePlaceView.swift` — QR üretimi (CoreImage) + ShareLink. `QRScannerView.swift` — AVFoundation tarama + `ImportConfirmView` + freemium gate.
...
- ⚠️ `PinlyLiveActivity.swift` ve `PinlyLiveActivityLiveActivity.swift` KULLANILMAYAN Xcode şablon artıkları (emoji örneği) — bundle'a kayıtlı değiller.
...
- Otomatik test yok; ilk hedefler ROADMAP §2.3'te (PlaceImporter round-trip, LocalEntitlementService, DefaultBadgeService.check, PlacesListViewModel, RouteManager hizalama).
...
```
TabView (HomeView)
  ├── Ana (house.fill)         — MainTab: istatistik şeridi + hero CTA + hızlı aksiyonlar + son eklenenler
  ├── Keşfet (grid)            — DiscoverView: kategorilere göre mekanlar
  ├── Rotalar (bookmark.map)   — SavedRoutesView: önceden planlanmış rotalar
  └── Daha Fazla (ellipsis)    — MoreTab: İstatistikler, Geçmiş, Haftalık Rapor, Rozetler, Dil
```
...
- [ ] Unit test paketi (ROADMAP §2.3) + GitHub Actions CI
